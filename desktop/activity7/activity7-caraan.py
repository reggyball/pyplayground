...
with open ("stop_words.txt", "r", encoding="utf-8") as file:                                #Text file is opened at read mode with UTF-8 encoding to ensure program reads text files with any character                              
    for line in file:                                                                       #text file is read per line
        ...
        stopwords_list.append(stop_word)                                                    #each stop word is appended in the empty stop_words list
...
with open("article.txt", "r") as file:                                                      #Text file is opended at read mode, automatically closed after use
...
#to remove special characters and punctuations in word
           ...
           for letter in word:                                                                 #Each letter of the word will be assesed if in alphabet list, 
                if letter in elphaba_list:                                                            #if yes each letter will be concatenated to 'term' string 
                    term += letter                                                                      

#check if term is in stop_words                                                                                                       
           if term in stopwords_list:                                                          #if term is not in stop_words list then it will be appended to the key_list
               continue                                                                        #key_list will contain all the valid terms in the document (words less the punctuation and special chars and not in stop words)
           else:
